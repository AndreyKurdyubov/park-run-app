...
#####################################################################################################
st.header('дети-волонтеры до 14 лет')
st.markdown('Здесь не учитывается только волонтеры')

querie = '''
WITH aProfs as (SELECT profile_link, name
    FROM runners 
    WHERE profile_link LIKE "%userstats%"
    UNION ALL
...
SELECT distinct a.profile_link, a.name, ag as "Группа", u.peterhof_finishes_count, u.peterhof_volunteers_count
FROM aProfs a
LEFT JOIN USERS u
    ON a.profile_link = u.profile_link
LEFT JOIN max_ag
    ON max_ag.profile_link = a.profile_link

WHERE (ag LIKE "%10%" OR ag is Null) AND u.peterhof_volunteers_count > 0
